...
import randomized_layer_normalization
import reader
...
class Model(object):
        ...
        for i in range(self.batch_size):
            ...
            for j in range(SEQ_LENGTH):
                ...
                def process_obj_mask(obj_id):
                    ...
                    mask = tf.logical_and(
                        tf.equal(
                            current_seg, obj_id),  # pylint: disable=cell-var-from-loop
                    ...
                    mask = tf.logical_and(
                        mask, tf.greater(size, MIN_OBJECT_AREA))
                    if not self.boxify:
                        return mask
                    # Complete the mask to its bounding box.
                    binary_obj_masks_y = tf.reduce_any(
                        mask, axis=1, keepdims=True)
                    binary_obj_masks_x = tf.reduce_any(
    ...
    def _build_summaries(self):
        ...
        if self.depth_consistency_loss_weight > 0:
            tf.summary.scalar('depth_consistency_loss',
                              self.depth_consistency_loss)
    ...
    def _build_depth_test_graph(self):
        ...
        with tf.variable_scope(DEPTH_SCOPE, reuse=tf.AUTO_REUSE):
            ...
            if self.imagenet_norm:
                input_image = (
                    input_image - reader.IMAGENET_MEAN) / reader.IMAGENET_SD
